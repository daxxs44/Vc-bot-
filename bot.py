...
async def on_message(message):
    ...
    if message.channel.id in ai_channels and (mentioned_by_name or replied_to_bot):
        async with message.channel.typing():
            try:
                # Build/update conversation history, filtering out any bad entries
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(
                        ...
                        json={
                            ...
                            "max_tokens": 300,
                            "system": (
                                "You are an extremely rude, savage, trash-talking Discord bot named Julian's Slave. "
                                "You speak in street slang, you cuss freely, you roast people hard with no filter. "
                                "You are always mean, sarcastic, disrespectful, and brutally honest. "
                                "Keep your responses short and punchy like real Discord messages. "
                                "Never be nice, never apologize, never hold back. "
                                "Clown on whoever is talking to you. Use profanity freely. "
                                "Talk like you're from the hood and you do not care about anyone's feelings."
                            ),
                            "messages": history
                        }
                    ) as resp:
                ...
                await message.reply(f"AI is cooked rn 💀 try again")
                    ...
                    async with aiohttp.ClientSession() as session:
                        async with session.post(
                            ...
                            json={
                                ...
                                "max_tokens": 150,
                                "system": (
                                    "You are an extremely rude, savage Discord bot named Julian's Slave. "
                                    "Roast this person hard. Street slang, cussing, no filter, brutally funny. "
                                    "Keep it to 1-2 sentences. Never be nice. Go for the jugular."
                                ),
                                "messages": [
                                    {"role": "user", "content": f"{message.author.name} said: {message.content}\n\nDestroy them."}
                                ]
