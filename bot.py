            ...
            try:
                ...
                for m in raw_history:
                    try:
                        ...
                        pass
                # Build the new user message, supporting text and images
                ...
                if image_urls:
                    ...
                    msg_parts = []
                    if safe_content:
                        msg_parts.append({"type": "text", "text": f"{safe_name}: {safe_content}"})
                    else:
                        msg_parts.append({"type": "text", "text": f"{safe_name} sent an image:"})
                ...
                async with session.post(
                    ...
                    json={
                        ...
                        "messages": [
                            ...
                            {"role": "user", "content": (
                                [
                                    {"type": "text", "text": f"{str(message.author.name or 'user')}: {str(message.content or 'sent an image')}"},
                                ] + [
                                    {"type": "image_url", "image_url": {"url": a.url}}
                                    for a in message.attachments if a.content_type and a.content_type.startswith("image/")
                                ]
                            ) if message.attachments else f"{str(message.author.name or 'user')}: {str(message.content or '')}"}
                        ]
                    }
                ) as resp:
                        ...
                        async with session.post(
                            ...
                            json={
                                ...
                                "messages": [
                                    ...
                                    {"role": "user", "content": (
                                        [
                                            {"type": "text", "text": f"{str(message.author.name or 'user')} said: {str(message.content or 'sent this image')}. Destroy them."},
                                        ] + [
                                            {"type": "image_url", "image_url": {"url": a.url}}
                                            for a in message.attachments if a.content_type and a.content_type.startswith("image/")
                                        ]
                                    ) if message.attachments else f"{str(message.author.name or 'user')} said: {str(message.content or '')}. Destroy them."}
                                ]
                            }
                        ) as resp:
                            data = await resp.json()
