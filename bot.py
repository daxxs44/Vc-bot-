...
async def on_voice_state_update(member, before, after):
    ...
    # Check if a punished user left voice - clean up VCs
    if not after.channel and before.channel and member.id in punished_users:
        if member.id in punishment_tasks:
            task, vc_ids = punishment_tasks[member.id]
            # Don't cancel task, let it clean up naturally
            # The VCs will be deleted by the punishment_loop when it detects user left
