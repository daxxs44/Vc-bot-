...
async def on_message(message):
    ...
    # If the AI is active in this channel, only respond when called by name or replied to
    bot_names = ["julian's slave", "julians slave", "julian slave"]
    mentioned_by_name = any(name in message.content.lower() for name in bot_names)
    replied_to_bot = (
    ...
    if message.channel.id in ai_channels and (mentioned_by_name or replied_to_bot):
        async with message.channel.typing():
            try:
                # Build/update conversation history for this channel (keep last 20 for context)
                history = ai_history.get(message.channel.id, [])
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(
                        ...
                        json={
                            ...
                            "max_tokens": 300,
                            "system": (
                                "You are an extremely rude, savage, trash-talking Discord bot. "
                ...
                await message.reply(f"AI broke lil bro 💀 ({e})")
